...
DEFAULT_DATA: Dict[str, Any] = {
    ...
    "tariffs": [
        ...
        {"id": 5, "name": "1 minut", "days": 1, "price": 7770, "is_active": True},
    ]
}
...
def get_tariff_keyboard(tariff_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        # [InlineKeyboardButton(text="✅ TO'LOV QILDIM", callback_data=f"pay_{tariff_id}")],
...
def get_admin_pay_approval_keyboard(pay_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="15 KUN", callback_data=f"adm_app_{pay_id}_15"),
            InlineKeyboardButton(text="30 KUN", callback_data=f"adm_app_{pay_id}_30")
        ],
        [
            InlineKeyboardButton(text="60 KUN", callback_data=f"adm_app_{pay_id}_60"),
            InlineKeyboardButton(text="90 KUN", callback_data=f"adm_app_{pay_id}_90")
        ],
        [
                    InlineKeyboardButton(text="1 minut", callback_data=f"adm_app_{pay_id}_1"),
        ],
        [
...
async def cb_user_profile(callback: CallbackQuery):
    ...
    if sub:
        ...
        text = (
            ...
            "Sizda hozirda faol VIP obuna mavjud emas. Obuna sotib olish uchun bosh menyudan tarif tanlang."
        )
...
async def cb_tariff_details(callback: CallbackQuery):
    ...
    text = (
        ...
        f"👤 Karta egasi:\n<b>{CARD_HOLDER}</b>\n\n"
        "⚠️ To'lovni amalga oshirgandan keyin chekni botga yuboring.\n\n"
        f"⏱ Chekni {PAYMENT_TIMEOUT_MINUTES} daqiqa ichida yuborish kerak."
    )
    await callback.message.edit_text(text, reply_markup=get_tariff_keyboard(tariff_id), parse_mode="HTML")
    await callback.answer()


@router.callback_query(F.data.startswith("pay_"))
async def cb_pay_create(callback: CallbackQuery):
    tariff_id = int(callback.data.split("_")[1])
    user_id = callback.from_user.id
    now = now_dt()
    ...
    text = (
        ...
        f"Karta: <code>{CARD_NUMBER}</code> ({CARD_HOLDER})\n"
        f"Summa: <b>{tariff['price']:,} so'm</b>\n\n"
        "To'lovdan keyin chekni shu botga <b>rasm sifatida</b> yuboring.\n\n"
        f"⏱ Sizda <b>{PAYMENT_TIMEOUT_MINUTES} daqiqa</b> vaqt bor."
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
...
async def cb_admin_approve_payment(callback: CallbackQuery):
    ...
    async with data_lock:
        ...
        db["admin_logs"].append({
            ...
            "details": f"{pay_id} to'lovi {days} kunga tasdiqlandi. Sub ID: {sub_id}",
            "created_at": now_iso()
        })
    ...
    if CHANNEL_ID != 0:
        try:
            ...
            logger.info(f"Join request mavjud emas yoki tasdiqlanmadi ({e}), invite link yaratiladi.")

    if approved_via_req:
        user_msg = (
            "🎉 <b>To'lovingiz tasdiqlandi!</b>\n\n"
            f"💎 VIP obunangiz faollashtirildi ({days} kun).\n"
            "Sizning kanalga qo'shilish so'rovingiz qabul qilindi.\n\n"
    ...
    await callback.answer(f"To'lov {days} kunga tasdiqlandi!")
...
async def cb_sub_history(callback: CallbackQuery):
    ...
    if not subs:
        ...
        for s in subs:
            st = format_date(parse_iso(s["start_date"]))
            en = format_date(parse_iso(s["end_date"]))
...
async def process_manual_sub_user_id(message: Message, state: FSMContext):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="15 KUN", callback_data="man_sub_15"),
            InlineKeyboardButton(text="30 KUN", callback_data="man_sub_30")
        ],
        [
            InlineKeyboardButton(text="60 KUN", callback_data="man_sub_60"),
            InlineKeyboardButton(text="90 KUN", callback_data="man_sub_90")
        ],
        [
            InlineKeyboardButton(text="◀️ BEKOR QILISH", callback_data="admin_panel_back")
        ]
    ])
    await message.answer(f"ID: <code>{target_id}</code> ga qancha muddatga obuna bermoqchisiz?", reply_markup=keyboard, parse_mode="HTML")
...
async def cb_give_sub_direct(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"ID: <code>{target_id}</code> ga muddatni tanlang:", reply_markup=keyboard, parse_mode="HTML")
    await callback.answer()
...
async def process_manual_sub_days(callback: CallbackQuery, state: FSMContext):
    ...
    async with data_lock:
        ...
        db["admin_logs"].append({
            ...
            "details": f"Admin qo'lda {days} kunlik obuna berdi. Sub ID: {sub_id}",
            "created_at": now_iso()
        })
        save_data_sync(db)
    ...
    if approved_via_req:
        try:
            await bot.send_message(
                chat_id=target_id,
                text=f"🎉 <b>Admin tomonidan sizga {days} kunlik VIP obuna taqdim etildi!</b>\n\nKanalga xush kelibsiz! 🍿",
                parse_mode="HTML"
            )
        except Exception:
            pass
        ...
        user_text = f"🎉 <b>Admin tomonidan sizga {days} kunlik VIP obuna taqdim etildi!</b>\n"
        keyboard = None
        if invite_url:
    ...
    await callback.message.edit_text(
        f"✅ ID <code>{target_id}</code> ga {days} kunlik obuna muvaffaqiyatli berildi!",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="◀️ ADMIN PANEL", callback_data="admin_panel_back")]
        ]),
        parse_mode="HTML"
...
# --- CANCEL SUBSCRIPTION & KICK HANDLERS ---
...
async def cb_confirm_cancel_sub(callback: CallbackQuery):
    ...
    async with data_lock:
        ...
        db["admin_logs"].append({
            ...
            "target_user_id": target_id,
            "details": f"Obuna admin tomonidan bekor qilindi",
            "created_at": now_iso()
        })
        save_data_sync(db)
...
async def cb_admin_kick_user(callback: CallbackQuery):
    ...
    async with data_lock:
        ...
        db["admin_logs"].append({
            ...
            "action": "KICK_USER",
            "target_user_id": target_id,
            "details": f"Foydalanuvchi kanaldan chiqarildi",
            "created_at": now_iso()
        })
        save_data_sync(db)
...
async def process_broadcast_content(message: Message, state: FSMContext):
    ...
    confirm_text = (
        ...
        f"👥 Qabul qiluvchilar: <b>{total_recipients}</b>\n\n"
            ...
            async with data_lock:
                ...
                for sub in subscriptions:
                    if sub.get("status") == "ACTIVE" and sub.get("is_active", False):
                        ...
                        elif timedelta(seconds=0) < time_left <= timedelta(hours=1):
...
async def main():
    ...
    # Boshlang'ich JSON faylini tekshirish va yaratish
    async with data_lock:
        load_data_sync()
    ...
    # Eskirgan update larni o'chirib pollingni boshlash
